...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(email=email, password=generate_password_hash(form.password.data, method="pbkdf2:sha256", salt_length=8), name=form.name.data)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            user_correct_password = user.password
            if check_password_hash(user_correct_password, form.password.data):
                login_user(user)
                return redirect(url_for("get_all_posts"))
            else:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            return redirect(url_for("login"))
    return render_template("post.html", post=requested_post, form=form, id=post_id, current_user=current_user, users=all_users)
